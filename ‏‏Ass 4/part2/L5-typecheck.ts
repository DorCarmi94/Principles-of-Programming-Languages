...
import { isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
         ...
         Parsed, PrimOp, ProcExp, Program, StrExp, isValuesExp, isLetValuesExp, isLetValuesBinding, ValuesExp, CExp, LetValuesExp, LetValuesBinding, VarDecl } from "./L5-ast";
import { applyTEnv, makeEmptyTEnv, makeExtendTEnv, TEnv } from "./TEnv";
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         parseTE, unparseTExp,
         BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, makeEmptyTupleTExp, makeNonEmptyTupleTExp, NonTupleTExp, isNonTupleTExp, EmptyTupleTExp, isEmptyTupleTExp, TupleTExp, isNonEmptyTupleTExp } from "./TExp";
...
import { applicativeEval, evalSequence } from './L5-eval';
import { env } from 'process';
import { SExpValue, Value, isEmptySExp } from './L5-value';
...
export const typeofValuesExp=(values:ValuesExp,tenv:TEnv):Result<TExp>=>
{
    if(values.values.length==0)
    {
        ...
        const myTypes=mapResult((x:CExp)=>typeofExp(x,tenv),values.values);
        const oded=bind(myTypes,(x:TExp[])=>mapResult((y:TExp)=>makeNonEmptyTupleFromTexp(y),x));
        return bind(oded,(o:NonTupleTExp[])=>makeOk(makeNonEmptyTupleTExp(o)));
        //NonTupleTExp[]
        //return bind(myTypes,(x:TExp[])=>makeOk(makeNonEmptyTupleFromTexp(x)));
    }
}

const makeNonEmptyTupleFromTexp=(exp:TExp):Result<NonTupleTExp>=>
    isNonTupleTExp(exp)?makeOk(exp):
    makeFailure("not non tuple");
...
export const typeofLetValuesExp=(exp:LetValuesExp,tenv:TEnv):Result<TExp>=>
{
    ...
    const flatVarsStrings=flatVars.map((v)=>v.var);
    const varTEs = map((b) => b.texp, flatVars);
    const vals=mapResult((v : CExp) => typeofExp(v, tenv), map((b : LetValuesBinding) => b.val, exp.bindings));
    const valuesInDoubleArray=bind(vals,(x:TExp[])=>mapResult((s:TExp)=>evalValuesArray(s),x));
    const flatValues=bind(valuesInDoubleArray,(x:TExp[][])=>makeOk(x.reduce((acc:TExp[],cur:TExp[])=>acc.concat(cur),[])));
    //const vars = map((b: VarDecl) => b.var, exp.vars);
//  const a=bind(valsCExp,(vals:SExpValue)=>evalValsVars(vars,vals,env));
    const constraints =bind(flatValues,(x:TExp[])=>zipWithResult((varTE, val) =>checkEqualType(varTE, val, exp), varTEs, x));
    return bind(constraints, _ => typeofExps(exp.body, makeExtendTEnv(flatVarsStrings, varTEs, tenv)));
}

const evalValuesArray=(exp:TExp):Result<TExp[]>=>
{
    
    return isEmptyTupleTExp(exp)?makeOk([exp]):
    isNonEmptyTupleTExp(exp)&&isArray(exp.TEs)?makeOk(exp.TEs):
    makeFailure("not a tuple type");
    
}


// const evalValuesArray=(exp:TExp):Result<EmptyTupleTExp[]|NonTupleTExp[]>=>
// {
//     return isEmptyTupleTExp(exp)?makeOk([exp]):
//     isNonTupleTExp(exp)&&isArray(exp)?makeOk(exp):
//     makeFailure("not a tuple type");
