...
// tuple()
const parseCompoundTExp = (texps: Sexp[]): Result<TupleTExp|ProcTExp> => {
    const pos = texps.indexOf('->');
    return (pos === -1)  ? parseCompundTupleTExp(texps) :
...
const parseCompundTupleTExp= (texp:Sexp[]):Result<TupleTExp>=>
    bind(parseMyTupleTExp(texp),(x:EmptyTupleTExp|NonTupleTExp[])=>makeOk(makeTupleTExp(x)));

const makeTupleTExp=(x:EmptyTupleTExp|NonTupleTExp[]):TupleTExp=>
    isEmptyTupleTExp(x)?x: 
    makeNonEmptyTupleTExp(x);
/*
;; Expected structure: <te1> [* <te2> ... * <ten>]?
;; Or: Empty
*/
const parseMyTupleTExp = (texps: Sexp[]): Result<EmptyTupleTExp|NonTupleTExp[]> => {
    ...
    return isEmptyTuple(texps) ? makeOk(makeEmptyTupleTExp()): bind(splitEvenOdds(texps),
                                                   (argTEs: Sexp[]) => mapResult(parseMyOdedTupleTExp, argTEs));
}

const parseMyOdedTupleTExp= (arg:Sexp):Result<NonTupleTExp>=>
{
    const a=parseTExp(arg);
    if(isOk(a))
    {
        return oded(a.value);
    }
...
const oded=(a:TExp):Result<NonTupleTExp>=>
    isNonTupleTExp(a)? makeOk(a):
    makeFailure("Not a non tuple");
...
export const unparseTExp = (te: TExp): Result<string> => {
    ...
    const up = (x?: TExp): Result<string | string[]> =>
        ...
        isTupleTExp(x) && isNonEmptyTupleTExp(x)? unparseTuple(x.TEs) :
