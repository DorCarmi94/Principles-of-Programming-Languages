...
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp|LetValuesExp | LitExp | LetrecExp | SetExp|ValuesExp;
export const isCompoundExp = (x: any): x is CompoundExp =>
    isAppExp(x) || isIfExp(x) || isProcExp(x) || isLitExp(x) ||isLetValuesExp(x)|| isLetExp(x) || isLetrecExp(x) || isSetExp(x)||isValuesExp(x);
export const expComponents = (e: Exp): CExp[] =>
    ...
    isSetExp(e) ? [e.val] :
    isDefineExp(e) ? [e.val] :
    isValuesExp(e) ? e.values:
    //todo:check
    isLetValuesExp(e)?[...e.body]:
...
export interface ValuesExp {tag: "ValuesExp"; values:CExp[];}
export const makeValuesExp= (vals: CExp[]): ValuesExp =>
({tag: "ValuesExp", values: vals});
...
export const parseValuesExp=(params:Sexp[]):Result<ValuesExp>=>
    ...
    isArray(params)? bind(mapResult(parseL5CExp, params),
    (cexps: CExp[]) => makeOk(makeValuesExp(cexps))):
    makeFailure("ZazziiiBazazziii");

//let-values(new)

// const parseLetValues=(bindings: Sexp, body: Sexp[]): Result<LetValuesExp>=>
// isArray(bindings)?parseLetValuesBindigns(first(bindings),body):
// makeFailure("bad parse let values exp");
...
safe2((bdgs:LetValuesBinding[], body: CExp[]) => makeOk(makeLetValuesExp(bdgs, body)))
        (mapResult(parseLetValBindingBinding,bindings), mapResult(parseL5CExp, body)):
        makeFailure("must be array");

const parseLetValBindingBinding=(bindingSexp:Sexp):Result<LetValuesBinding>=>
    !isArray(bindingSexp)? makeFailure("must be an array"):
    !isArray(bindingSexp[0])? makeFailure("must be an array"):
    safe2((vds: VarDecl[], vals: CExp) => makeOk(makeLetValuesBinding(vds, vals)))
    (mapResult(parseVarDecl,bindingSexp[0]),parseL5CExp(bindingSexp[1]));
...
//const exps:Result<CExp[]>=mapResult(parseL5CExp,params);
    // //const varDecls:Result<VarDecl[]>=mapResult(makeVarDecl())
    // let oded:[Sexp, Sexp][];
...
const unparseVarDeclArray=(vardecls:VarDecl[]):Result<string>=>
{
    const oded=mapResult((v:VarDecl)=>unparseVarDecl(v),vardecls)
    return bind(oded,(x:string[])=>makeOk(`(${join(" ", x)})`));
}

const unparseValuesBindigns=(bindings:LetValuesBinding[]):Result<string>=>
    bind(mapResult(bdg => safe2((vd: string, val: string) => makeOk(`(${vd} ${val})`))(unparseVarDeclArray(bdg.var), unparse(bdg.val)), bindings),
    (bdgs: string[]) => makeOk(join(" ", bdgs)));   
...
const unparseValuesExp=(ve:ValuesExp):Result<string>=>
    bind(mapResult(unparse, ve.values), (les: string[]) => makeOk(`(values ${join(" ", les)})`));


//(new) let-values
// const unparseLetValues=(lve:LetValuesExp):Result<string>=>
//     safe2((bdgs: string, body: string) => makeOk(`(let (${bdgs}) ${body})`))
//     (unparseBindings(le.bindings), unparseLExps(.body));    
...
const unparseLetValues=(le:LetValuesExp):Result<string>=>
    safe2((bdgs: string, body: string) => makeOk(`(let-values (${bdgs}) ${body})`))
        (unparseValuesBindigns(le.bindings), unparseLExps(le.body));
