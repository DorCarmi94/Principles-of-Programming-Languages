...
//L21
//varDecl: VarDecl; startValue: NumExp; endValue: NumExp; body: CExp; };
export const makeForExp = (varDecl: VarDecl, startValue: NumExp, endValue: NumExp, body: CExp): ForExp =>
    ({tag: "ForExp", var: varDecl, start: startValue, end: endValue,body:body});  
...
//export interface ForExp {tag: "ForExp"; varDecl: VarDecl; startValue: NumExp; endValue: NumExp; body: CExp; };
...
const parseForExp = (params: Sexp[]): Result<ForExp> =>
    params.length===4?parseGoodFor(params[0],params[1],params[2],params[3]):makeFailure("Zazziiibazaziii");
    // safe3((start:CExp,end:CExp,body:CExp)=>parseGoodFor(params[0],start,end,body))
    //     (parseL21CExp(params[1]),parseL21CExp(params[2]),parseL21CExp(params[3]));
    
//const funktsia()

const parseGoodFor=(identifier: Sexp, numStartVal: Sexp, numEndVal:Sexp, body: Sexp): Result<ForExp> =>
    !isIdentifier(identifier)?makeFailure("First arg of for must be an identifier") :
    !Number(numStartVal)? makeFailure("Second arg of for must be an number"+numStartVal):
    !Number(numEndVal)?makeFailure("Third arg of for must be an number") :
    bind(parseL21CExp(body),(x:CExp)=>makeOk(makeForExp(makeVarDecl(identifier),makeNumExp(+numStartVal),makeNumExp(+numEndVal),x)));
...
const safe4 = <T1, T2, T3, T4, T5>(f: (x: T1, y: T2, z: T3, w:T4) => Result<T5>): (xr: Result<T1>, yr: Result<T2>, zr: Result<T3>, wr:Result<T4>) => Result<T5> =>
